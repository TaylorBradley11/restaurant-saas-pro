  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0e0c", fontFamily: "var(--sans)", color: "#faf8f4" }}>
      ...
      <nav style={{
        ...
        <Link href="/" style={{ fontFamily: "var(--serif)", fontSize: 22, fontWeight: 300, letterSpacing: "0.08em", color: "#d4b896" }}>
          SHA MUU
        </Link>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button
            ...
            style={{
              background: cart.length > 0 ? "#b8966e" : "rgba(255,255,255,0.07)",
              color: cart.length > 0 ? "#0f0e0c" : "rgba(255,255,255,0.4)",
              border: "none", borderRadius: 20, padding: "8px 18px",
              fontSize: 12, letterSpacing: "0.08em", cursor: "pointer",
              fontFamily: "var(--sans)", fontWeight: 500, transition: "all 0.2s"
            }}
          >
      ...
      {/* Subtitle bar */}
      <div style={{ background: "linear-gradient(135deg, #1a1510 0%, #0f0e0c 100%)", padding: "24px 32px 0" }}>
        <h1 style={{ fontFamily: "var(--serif)", fontSize: 13, fontWeight: 300, letterSpacing: "0.2em", textTransform: "uppercase", color: "#b8966e", marginBottom: 4 }}>
          Order Online
        </h1>
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.2)", letterSpacing: "0.06em", marginBottom: 24 }}>
          23 North 900 West · Salt Lake City · Free delivery
        </p>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 32px" }}>
        <h2 style={{ fontFamily: "var(--serif)", fontSize: 40, fontWeight: 300, color: "#faf8f4", marginBottom: 4, letterSpacing: "-0.01em" }}>
          {activeCategory}
        </h2>
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.25)", letterSpacing: "0.08em", marginBottom: 32 }}>
            ...
            {currentItems.map(item => (
              <div
                ...
                style={{
                  ...
                  border: justAdded === item.id ? "1px solid rgba(184,150,110,0.7)" : "1px solid rgba(255,255,255,0.07)",
                  borderRadius: 14,
                  padding: "20px",
                  display: "flex",
                  gap: 16,
                  ...
                  transform: justAdded === item.id ? "scale(1.02)" : "scale(1)",
                }}
              >
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: "var(--serif)", fontSize: 16, color: "#faf8f4", marginBottom: 4, lineHeight: 1.3 }}>
                    {item.name}
                  </div>
                  {item.description && (
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10 }}>
                    <span style={{ fontFamily: "var(--serif)", fontSize: 17, color: "#d4b896" }}>
                      ${item.price.toFixed(2)}
                    </span>
                    <button
                      onClick={() => add(item)}
                      style={{
                        background: justAdded === item.id ? "#d4b896" : "rgba(184,150,110,0.12)",
      ...
      {cart.length > 0 && !cartOpen && (
        <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", zIndex: 150 }}>
          <button
            onClick={() => setCartOpen(true)}
            style={{
              background: "#b8966e", color: "#0f0e0c",
              border: "none", borderRadius: 30,
              padding: "14px 32px", fontSize: 13,
              fontWeight: 600, letterSpacing: "0.08em",
              cursor: "pointer", fontFamily: "var(--sans)",
              boxShadow: "0 8px 32px rgba(184,150,110,0.5)",
              whiteSpace: "nowrap",
            }}
          >
      ...
      {cartOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200 }}>
          ...
          <div style={{
            ...
            borderLeft: "1px solid rgba(184,150,110,0.15)",
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
              {cart.length === 0 ? (
                ...
                <div key={idx} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(184,150,110,0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
                    {item.emoji}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "var(--serif)", fontSize: 14, color: "#faf8f4", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {item.name}
                    </div>
            ...
            {cart.length > 0 && (
              <div style={{ padding: "20px 28px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <button
                  onClick={checkout}
                  style={{ width: "100%", background: "#b8966e", color: "#0f0e0c", border: "none", borderRadius: 10, padding: "16px 0", fontSize: 13, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 600, cursor: "pointer", fontFamily: "var(--sans)" }}
                >
                  Checkout →
                </button>
                <button
                  onClick={() => setCartOpen(false)}
                  style={{ width: "100%", background: "transparent", color: "rgba(255,255,255,0.2)", border: "none", padding: "12px 0", fontSize: 11, cursor: "pointer", fontFamily: "var(--sans)", marginTop: 8 }}
                >
                  Continue ordering
                </button>
              </div>
